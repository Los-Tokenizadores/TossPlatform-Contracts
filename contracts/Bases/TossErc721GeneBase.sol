...
abstract contract TossErc721GeneBase is ITossSellErc721, TossErc721MarketBase {
    ...
    function __TossErc721GeneBase_init(string memory name_, string memory symbol_) initializer public {
    ...
    function addGenes(uint256[] memory genes) virtual external onlyRole(MINTER_ROLE) {
        for(uint256 i = 0; i < genes.length;) {
    ...
    function sellErc721(address _owner, uint8 _amount) override external onlyRole(MINTER_ROLE) {
