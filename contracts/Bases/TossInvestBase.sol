...
abstract contract TossInvestBase is TossWhitelistClient, PausableUpgradeable, AccessControlUpgradeable, TossUUPSUpgradeable {
    ...
    function __TossInvestBase_init(address erc20Address_, address erc721ImplementationAddress_, address platformAddress_, uint16 platformCut_, string memory erc721baseUri_) initializer public {
    ...
    function setErc721Implementation(address newAddress) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        require(IERC165Upgradeable(newAddress).supportsInterface(type(IERC721Upgradeable).interfaceId), "Contract does not support IERC721Upgradeable");
    ...
    function addProject(string memory name, string memory symbol, uint32 targetAmount, uint32 maxAmount, uint128 price, uint64 startAt, uint64 finishAt, address projectWallet) external onlyRole(PROJECT_ROLE) {
     ...
     function changeProject(uint256 projectId, string memory name, string memory symbol, uint32 targetAmount, uint32 maxAmount, uint128 price, uint64 startAt, uint64 finishAt, address projectWallet) external onlyRole(PROJECT_ROLE) {
        require(projectId < projects.length, "project not exist");
